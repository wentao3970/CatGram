...
// Used to handle upload and download data (other than user) from the Database
import Foundation
import SwiftUI
...
class DataService {
    ...
    private func getPostsFromSnapshot(querySnapshot: QuerySnapshot?) -> [PostModel]{
        ...
        if let snapshot = querySnapshot, snapshot.documents.count > 0 {
            ...
            print("No document in snapshot found for this user")
            return postArray
        }
    }
    ...
    func likePost(postID: String, currentUserID: String) {
        ...
        // Update the array of users who like this post
